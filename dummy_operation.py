...
def read_edge_format(file_path):
    ...
    if len(header) < 3:
        raise ValueError(f"Invalid header in {file_path}: {lines[0]}")
...
def create_var(num_ops, num_jobs, num_factories, request_list, job_of, E_star, ES, LS, last_ops, UB):
    s, x, f, m, xm, sf, sm, sfm = {}, {}, {}, {}, {}, {}, {}, {}
    counter = 0

    # 1. Biến thời gian cho các operation thực
    ...
    # Biến x duy nhất cho dummy operation
    dummy_op = num_ops
    ES_dummy = max(ES[u] + min(request_list[u].values()) for u in last_ops)
    for t in range(ES_dummy, UB + 2):
        counter += 1
        x[(dummy_op, t)] = counter
    ...
    return s, x, f, m, xm, sf, sm, sfm, counter, dummy_op, ES_dummy


def build_constraints(solver, num_ops, num_jobs, num_factories, precedence_list, request_list, job_of, E_star, ES, LS, first_ops, s, x, f, m, xm, sf, sm, sfm, args, last_ops, dummy_op, ES_dummy, UB):
    cnf = CNF()
    ...
    # Ràng buộc chuỗi thứ tự x cho dummy_op
    for t in range(ES_dummy, UB + 1):
        cnf.append([-x[(dummy_op, t + 1)], x[(dummy_op, t)]])

    # Ràng buộc precedence từ last_ops đến dummy_op
    for u in last_ops:
        for k, p_uk in request_list[u].items():
            for t in range(ES_dummy, UB + 2):
                target_t = t - p_uk
                if target_t > LS[u]:
                    ...
                    cnf.append([x[(dummy_op, t)], -m[(u, k)]])
                else:
                    cnf.append([x[(dummy_op, t)], -m[(u, k)], -x[(u, target_t)]])
...
def solve_and_print(num_ops, num_jobs, num_factories, precedence_list, request_list, job_of, E_star, initial_UB, args, real_start_time):
    ...
    s, x, f, m, xm, sf, sm, sfm, _, dummy_op, ES_dummy = create_var(
        num_ops, num_jobs, num_factories, request_list, job_of, E_star, ES, LS, last_ops, initial_UB
    )
    ...
    build_constraints(
        ...
        last_ops, dummy_op, ES_dummy, initial_UB
    )
    ...
    while current_UB >= 0:
        ...
        if current_UB + 1 < ES_dummy:
            print("   UNSAT (UB lower than minimum possible completion time)")
            break

        assumptions = [-x[(dummy_op, current_UB + 1)]]
...
def verify_schedule(schedule, precedence_list, C_max):
    ...
    if max_end > C_max:
        print(f" [X] Fault Makespan: End time ({max_end}) is over C_max ({C_max})")
        return False

    for u, v in precedence_list:
        if schedule[u]['end'] > schedule[v]['start']:
            print(f" [X] Fault Precedence: Op {u} end at {schedule[u]['end']} but Op {v} starts at {schedule[v]['start']}")
            return False
    ...
    for j_id, factories in job_factories.items():
        if len(factories) > 1:
            print(f" [X] Fault Factory: Job {j_id} is assigned to multiple factories: {factories}")
            return False
